...
<header id="astroid-header" class="<?php echo implode(' ', $class); ?>"<?php echo $headAttrs; ?>>
   <div class="d-flex flex-row justify-content-between">
      ...
      <?php if ($block_1_type != 'blank' || $mode == 'right' || $enable_offcanvas) : ?>
         <div class="header-right-section as-gutter-x-xl@lg <?php echo ($enable_offcanvas ? 'd-flex' : 'd-'.$header_breakpoint.'-flex d-none'); ?> justify-content-end<?php echo $mode == 'right' ? ' flex-'.$header_breakpoint.'-grow-1' : ''; ?>">
            <?php
            if ($mode == 'right') {
               // header nav starts
                if ($header_menu_method == 'module_position') {
