...
$htag    = $article_params->get('show_page_heading') ? 'h2' : 'h1';
...
$canEdit = $article_params->get('access-edit');
if ($article_params->get('show_title')) {
    ...
    if (!empty($title_font_style)) {
        Style::renderTypography('#' . $element->id . ' h1, #' . $element->id . ' h2, #' . $element->id . ' h3, #' . $element->id . ' h4, #' . $element->id . ' h5, #' . $element->id . ' h6', $title_font_style, null, $element->isRoot);
    }
}
if ($canEdit) {
    echo LayoutHelper::render('joomla.content.icons', ['params' => $article_params, 'item' => $item->article]);
}
