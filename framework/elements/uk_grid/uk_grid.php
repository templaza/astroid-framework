...
foreach ($grids->data as $key => $grid) {
    ...
    if ($grid->params->get('type', '') == 'image' && $grid->params->get('image', '')) {
        ...
        $media      .=  '<img class="tz-img-grid '.$hover_effect.$img_transition.$img_tog_class  . ($image_fullwidth ? 'w-100' : '') . ($enable_image_cover || $media_position == 'left' || $media_position == 'right' ? ' object-fit-cover h-100' : '') . ($params->get('card_style', '') == 'none' ? '' : ' card-img-'. $media_position) .'" src="'. Astroid\Helper\Media::getMediaPath($grid->params->get('image', '')) .'" alt="'.$grid->params->get('title', '').'">';
    ...
    echo '<div id="grid-'. $grid -> id .'" class="as-grid"><div class="card' .$hover_tog_class. $card_style . $box_shadow . $box_shadow_hover .$bd_radius . $card_hover_transition . ($enable_grid_match ? ' h-100' : '') . '">';
